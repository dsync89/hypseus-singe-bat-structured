...
float2 VideoSize()
{
	...
	float2 video_size = in_video_size;

	// 0 can be set when the video and screen sizes are the same.
	if ((in_video_size.x == 0) && (in_video_size.y == 0))	// stretched game
	{
	...
	{
		if (in_video_size.x == 0)
			video_size.x = stretch_ratio * video_size.y;
		if (in_video_size.y == 0)
...
bool PIXELTESTS()
{
	...
	if ((in_video_size.x == 0) && (in_video_size.y == 0))	// stretched game (pixel test positions should be defined in 1920x1080)
	{
